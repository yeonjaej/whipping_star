			...
			if( this->eventSelection(j) ){
				for(int t=0; t<branch_variables.at(j).size();t++){
					...
					if(branch_variables.at(j).at(t)->getOscillate()){
						...
						double osc_probability_sin = nu_model.oscProbSin(true_var, true_L/float(1000));
						double osc_probability_sinsq = nu_model.oscProbSinSq(true_var, true_L/float(1000));
